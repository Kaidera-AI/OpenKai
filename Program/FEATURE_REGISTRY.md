...
| `upgrade` bun channel (managed) | 0.1.9 | ✅ | `isBunManaged()` detects a bun install via `~/.bun/` in `process.execPath` OR `process.argv[1]` (bin shim), then defers lifecycle to `bun add -g @kaidera/openkai`; `--check` read-only, `--rollback` refuses (bun owns history); upgrade.ts:92-107 (detect), 647-658 (dispatch), 604-614 (managed --check/--rollback refusal) — anchors re-derived at 3d0322b after fa187af hardened detection |
