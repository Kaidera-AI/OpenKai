...
**0.1.15 is the current release** (2026-09-17): npm `@kaidera/openkai` and `@kaidera/openkai-engine` (`latest`), the GitHub release with signed binaries for macOS, Linux and Windows plus `SHA256SUMS.txt`, the Homebrew tap (`kaidera-ai/tap/openkai`), the `bun add -g @kaidera/openkai` channel, and `scripts/install.sh` (default `v0.1.15`). All four distribution channels were independently re-verified end-to-end on 2026-09-17 after a Homebrew-tap deploy-key incident (see [Program/OpenKai/PROGRESS.md](Program/OpenKai/PROGRESS.md), private `kaideraos` repository). Releases ship only on explicit CTO consent per [RELEASE_SOP](docs/RELEASE_SOP.md).
