...
test("statusline config: defaults to all chips in canonical order", () => {
  ...
  assert.deepEqual([...DEFAULT_STATUSLINE_CHIPS], ["brand", "agent", "provider", "git", "persist", "session", "state", "plan", "ctx", "tokens", "model"]);
});
...
test("statusline config: STATUSLINE_CHIPS constant matches the spec set", () => {
  assert.deepEqual([...STATUSLINE_CHIPS], ["brand", "agent", "model", "session", "tokens", "persist", "provider", "state", "git", "ctx", "plan"]);
});
