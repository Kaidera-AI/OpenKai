...
test("golden-frame: overlay shows the P4a footer grammar and theme-token diff colours", async () => {
  ...
  // Theme-token diff colours: added uses the deterministic xterm-256 mint
  // fallback (152 = #AFD7D7); exact #B0E1CD is the truecolour mapping.
  assert.ok(rawFrame.includes("\x1b[38;5;152m"), "added diff lines use the mint fallback token (152)");
  ...
  await writeFile(
    ...
    [
      ...
      "# Diff colours: added=highlight.base(mint fallback 152 #AFD7D7) removed=highlight.danger(red 124)",
      "",
      raw.join("\n"),
      "",
    ].join("\n"),
