//# hash=661050fbf447cbff0c69626baafa2507
 ...
 */ /** Parse a submitted line into a slash command, or `null` if it isn't one. */ export function parseSlashCommand(line) {
    var trimmed = line.trim();
    if (!trimmed.startsWith("/")) return null;
    var space = trimmed.indexOf(" ");
    var name = space === -1 ? trimmed.slice(1) : trimmed.slice(1, space);
    var argument = space === -1 ? "" : trimmed.slice(space + 1).trim();
    return {
        name: name,
        argument: argument
    };
}
/** The P4 slash-command set, with descriptions for the autocomplete surface. */ export var SLASH_COMMANDS = [
    {
        name: "help",
    ...
    {
        name: "btw",
        description: "Ask a clarifying side question (answer as a system block, not a user turn).",
        argumentHint: "<text>"
    },
    {
        name: "undo",
        description: "Undo the last gated mutation (restore the previous shadow snapshot)."
    },
    {
        name: "quit",
...
/** Autocomplete items derived from the slash set (for the Editor surface). */ export function slashAutocompleteItems() {
    return SLASH_COMMANDS.map(function(c) {
        return {
            value: "/".concat(c.name),
            label: c.name,
...
/** The help text rendered by the `/help` command (plain lines). */ export function helpText() {
    return [
 ...
 */ export function buildPaletteItems(actions) {
    var items = [
        ...
        },
        {
            value: "sessions",
            label: "Sessions",
            description: "List local persisted sessions",
            keys: "/sessions"
        },
        {
            value: "resume",
        ...
        {
            value: "stash",
            label: "Stash / pop draft",
            description: "Stash or pop the prompt draft",
            keys: "Ctrl+S"
        }
    ];
