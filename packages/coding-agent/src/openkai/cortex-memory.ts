...
const cortexSearchBase: CustomTool<typeof SearchParams> = {
	...
	async execute(_id, params, _onUpdate, _ctx) {
		...
		if (client === undefined) {
			return {
				content: [{ type: "text", text: "cortex_search: local mode (no CORTEX_PROJECT) — nothing to search" }],
			};
		}
		try {
			...
			const body = results.map((r, i) => `${i + 1}. [${r.source}] ${r.text.slice(0, 400)}`).join("\n\n");
			return {
			...
			return {
				content: [
					{
						type: "text",
						text: `cortex_search failed: ${error instanceof Error ? error.message : String(error)}`,
					},
				],
				isError: true,
			};
...
const cortexRecordBase: CustomTool<typeof RecordParams> = {
	...
	async execute(_id, params, _onUpdate, _ctx) {
		...
		if (client === undefined) {
			return {
				content: [{ type: "text", text: "cortex_record: local mode (no CORTEX_PROJECT) — learning NOT recorded" }],
				isError: true,
			};
		}
		try {
			...
			return {
				content: [
					{
						type: "text",
						text: `cortex_record failed: ${error instanceof Error ? error.message : String(error)}`,
					},
				],
				isError: true,
			};
	...
	renderResult: (_result, _options, theme) => new Text(theme.fg("success", "◆ cortex recorded"), 1, 0),
};
