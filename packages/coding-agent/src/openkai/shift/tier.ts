...
const CRITICAL_PATTERNS = [/out of memory/i, /\bOOM\b/, /connection refused/i, /ENOSPC/i, /kernel panic/i];
...
function spinExplore(signals: ToolSignal[], turnDepth: number): { spinning: number; exploring: number } {
	...
	const reads = recent.filter(
		s => s.tool.startsWith("read") || s.tool === "grep" || s.tool === "glob" || s.tool === "list_files",
	).length;
