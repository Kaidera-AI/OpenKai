...
import { type ActivitySink, createRedactingSink, type RoutingEvent } from "./activity.js";
import type { Stage } from "./stages.js";
import { classifyStage, type ShiftInput, type StageConfig } from "./stages.js";
 ...
 * on the first retry.
 */
export function fallbackChain(stage: Stage, cast: Cast, fallbackCasts: Cast[] = []): FallbackTarget[] {
...
export class ShiftRouter {
	...
	next(stage: Stage, error: { status?: number; message: string }): RouteResult {
		...
		const isTransientNetwork =
			status === 0 && /ECONNRESET|ETIMEDOUT|ENOTFOUND|ECONNREFUSED|fetch failed/i.test(error.message);
		const isFallbackEligible = status === 429 || (status >= 500 && status < 600) || isTransientNetwork;
		...
		if (!isFallbackEligible) {
			...
			throw new Error(
				`routing failed for stage "${stage}" with non-retryable error ` + `(status ${status}): ${error.message}`,
			);
		}
 ...
 */
export function shiftRoute(input: ShiftInput, options: ShiftRouterOptions): { stage: Stage; result: RouteResult } {
