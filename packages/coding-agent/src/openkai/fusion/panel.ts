 ...
 */
export async function runPanel(streamFn: StreamFunction<Api>, options: PanelOptions): Promise<RoleOutput[]> {
	const prompt = options.sharedContext ? `${options.task}\n\n${options.sharedContext}` : options.task;
