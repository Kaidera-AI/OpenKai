 ...
 */
export async function readFusionRuns(logPath: string = defaultFusionLogPath()): Promise<FusionRunRecord[]> {
	let text: string;
	try {
...
export function summariseFusionRuns(records: FusionRunRecord[]): FusionPairStats[] {
	...
	for (const record of records) {
		const pair = record.roles.map(r => `${r.role}:${r.modelId}`).join(" + ") || "unknown";
	...
	for (const [pair, runs] of byPair) {
		...
		stats.push({
			pair,
			runs: runs.length,
			gatePassRate: gatedRuns.length > 0 ? passed / gatedRuns.length : undefined,
			avgWallMs: Math.round(runs.reduce((s, r) => s + r.wallMs, 0) / Math.max(runs.length, 1)),
			totalTokens,
			avgRoleLatencyMs: Math.round(roleLatencies.reduce((s, v) => s + v, 0) / Math.max(roleLatencies.length, 1)),
		});
	}
