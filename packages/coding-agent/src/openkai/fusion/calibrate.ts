 ...
 */
export const CALIBRATION_THRESHOLDS: readonly number[] = [0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8];
...
export function runCalibration(input: {
	...
	const capableQuality = paired === 0 ? 0 : [...pairedOutcomes.values()].filter(o => o.capable).length / paired;
	const efficientQuality = paired === 0 ? 0 : [...pairedOutcomes.values()].filter(o => o.efficient).length / paired;
	const qualityGap = capableQuality - efficientQuality;

	const escalates = (taskId: string, threshold: number): boolean => (scores.get(taskId) ?? 0) >= threshold;
	...
	const { recommendedThreshold, recommendationReason } = recommendThreshold(rows, quadrants.rescue.length);
 ...
 */
export function judgeBreakEvenLine(be: JudgeBreakEven, pair: { cheap: string; dear: string }): string {
...
export function renderCalibrationReport(
	...
	lines.push(
		`baselines — capable ${pct(report.capableQuality)} pass · ` + `efficient ${pct(report.efficientQuality)} pass`,
	);
	lines.push("");
	lines.push(
		col("threshold", 10) +
	...
	if (report.recommendedThreshold === undefined) {
		...
		lines.push(
			`recommendation: threshold ${report.recommendedThreshold.toFixed(2)} — ${report.recommendationReason}`,
		);
	}
