...
export class FusionBandit {
	...
	update(records: FusionRunRecord[], bucketOf: (r: FusionRunRecord) => ComplexityBucket): void {
		for (const record of records) {
			...
			if (OUTCOME_SUCCESS.has(record.gate.outcome) === false && OUTCOME_FAILURE.has(record.gate.outcome) === false)
				continue;
	 ...
	 */
	recommend(bucket: ComplexityBucket, candidates: string[]): BanditRecommendation | undefined {
