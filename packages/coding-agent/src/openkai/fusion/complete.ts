...
import type { Api, Context, Model, OptionsForApi, StreamFunction, Usage } from "@oh-my-pi/pi-ai";
...
export async function complete(
	...
	if (message.stopReason === "error" || message.stopReason === "aborted") {
		throw new Error(`completion failed (${message.stopReason}): ${message.errorMessage ?? "no detail"}`);
	}
	return {
