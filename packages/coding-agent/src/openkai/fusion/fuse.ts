...
import { designGate, gateListing, parseGateDesign, runGatedFusion, verbatimFailures } from "./gate.js";
import { newRunId } from "./helpers.js";
import { runPanel } from "./panel.js";
import { resolveSynthesiser, runSynthesis } from "./synthesis.js";
import type { FusionRunRecord, GateCheck, GateRun, RoleOutput, SynthesisArtifact } from "./types.js";
 ...
 */
export async function fuse(streamFn: StreamFunction<Api>, options: FuseOptions): Promise<FuseResult> {
	...
	const checks = gated ? await designGate(streamFn, judge, options.task) : undefined;
	...
	if (checks && synthesis.synthesisError === undefined) {
		const builderOutput = outputs.find(o => o.role === "builder")?.text ?? "";
		try {
