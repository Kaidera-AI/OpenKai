...
const isDiscard = (value: unknown): value is SynthesisArtifact["discarded"][number] => {
	if (!value || typeof value !== "object") return false;
	const d = value as Record<string, unknown>;
	return typeof d.item === "string" && typeof d.reason === "string" && (d.by === "architect" || d.by === "builder");
};
 ...
 */
function narrowItems<T>(items: unknown, guard: (value: unknown) => value is T, what: string): T[] {
	const list = Array.isArray(items) ? items : [];
	return list.map(item => {
		if (!guard(item)) {
			throw new AttributionError(`unattributed ${what} in synthesis: ${JSON.stringify(item).slice(0, 160)}`);
		}
		return item;
	});
}
