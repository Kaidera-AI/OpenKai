...
export function parseGateDesign(raw: string): GateCheck[] {
	...
	const checks = list.slice(0, 8).map(item => {
		if (!isGateCheck(item)) {
			throw new Error(`validator emitted a malformed check: ${JSON.stringify(item).slice(0, 160)}`);
		}
		return item;
	});
	if (checks.length === 0) {
...
function runCheck(check: GateCheck, options: RunGateOptions): Promise<GateCheckResult> {
...
export function gateListing(checks: GateCheck[]): string {
	const lines = checks.map((c, i) => `${i + 1}. ${c.name}: \`${c.command}\` (expect exit ${c.expectExit ?? 0})`);
 ...
 */
export async function runGatedFusion(options: GatedFusionOptions): Promise<{ runs: GateRun[]; finalWork: string }> {
	...
	for (let round = 1; round <= maxRounds; round += 1) {
		...
		const defective = evaluation.results.some(r => r.exitCode === 127 && !r.timedOut);
		if (defective && !evaluation.pass && gateRepairsUsed === 0 && options.repairGate) {
			gateRepairsUsed += 1;
			...
			const consented = options.approveGate ? await options.approveGate(repaired) : false;
			if (!consented) {
				throw new GateHaltError(
