...
const fusionToolBase: CustomTool<typeof FusionParams, FusionDetails> = {
	...
	async execute(_id, params, _onUpdate, ctx) {
				...
				content: [
					{
						type: "text",
						text:
							params.architectModel !== undefined
		...
		if (builder === undefined && params.builderModel === undefined) {
			...
			const candidates: PairCandidate[] = registry.getAvailable().map(m => ({ provider: m.provider, id: m.id }));
		...
		try {
			...
			return {
				content: [
					{ type: "text", text: `fusion failed: ${error instanceof Error ? error.message : String(error)}` },
				],
				isError: true,
			};
		}
	...
	renderResult: (result, _options, theme) => {
		...
		const lines = [
			...
			...details.roles.map(r =>
				theme.fg(
					r.error === undefined ? "success" : "error",
					`  ${r.role} · ${r.modelId} · ${r.latencyMs}ms${r.error !== undefined ? ` — ${r.error}` : ""}`,
				),
			),
			theme.fg(
				"muted",
				`  consensus ${details.consensusCount} · divergences ${details.divergenceCount} · gate ${details.gateOutcome}`,
			),
			...(details.pairSource !== undefined
				? [
						theme.fg(
							"dim",
							`  pair: ${details.pairSource}${details.pairAdvisory !== undefined ? ` — ${details.pairAdvisory}` : ""}`,
						),
					]
				: []),
		];
		...
		if (details.verificationChildId !== undefined) {
			...
			if (child !== undefined) {
				...
				const elapsed =
					pending !== undefined ? ` — ${Math.max(0, Math.round((Date.now() - pending.startedAt) / 1000))}s` : "";
				lines.push(
					theme.fg(
						"muted",
						pending !== undefined
