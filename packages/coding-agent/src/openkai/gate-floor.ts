...
/** The real (symlink-resolved) system temp root, cached. */
function tmpdirCanonical(): string {
	if (tmpdirCache === undefined) {
		try {
			tmpdirCache = fs.realpathSync(os.tmpdir());
		} catch {
			tmpdirCache = os.tmpdir();
		}
	}
	return tmpdirCache;
}
