...
const rlmSpawnBase: CustomTool<typeof SpawnParams> = {
  ...
  renderResult: (result, _options, theme) => {
    ...
    return new Text(theme.fg("accent", `◆ rlm child ${details?.childId ?? ""} admitted (${details?.model ?? "?"})`), 1, 0);
  },
};
...
const rlmCollectBase: CustomTool<typeof CollectParams> = {
  ...
  async execute(_id, params, _onUpdate, _ctx) {
    ...
    if (params.childId !== undefined) {
      ...
      return {
        content: [{ type: "text", text: `[${result.childId} · ${result.model}] ${result.text || `(failed: ${result.error ?? "unknown"})`}` }],
      };
    }
    const settled = registry.settledResults();
    const attribution = registry.childUsageAttribution();
    if (settled.length === 0) {
      return { content: [{ type: "text", text: "rlm_collect: no settled children yet (children never block — check again shortly)" }] };
    }
    const body = settled.map((r) => `[${r.childId} · ${r.model}] ${r.text.slice(0, 400)}`).join("\n\n");
    return {
      content: [{ type: "text", text: `rlm settled (${settled.length}):\n${body}\n\nchild usage attributed: ${attribution.totalTokens} tokens` }],
  ...
  renderResult: (result, _options, theme) => {
    ...
    return new Text(theme.fg("success", `◆ rlm collected${details?.settled !== undefined ? ` — ${details.settled} settled` : ""}`), 1, 0);
  },
};
