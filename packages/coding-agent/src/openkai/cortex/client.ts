...
export class CortexClient {
	...
	constructor(options: CortexClientOptions) {
		...
		this.baseUrl = (options.baseUrl ?? process.env.CORTEX_API_URL ?? DEFAULT_CORTEX_API_URL).replace(/\/+$/, "");
		this.project = options.project;
		this.agent = options.agent;
		this.fetchImpl = options.fetch ?? fetch;
		this.token = process.env.CORTEX_API_TOKEN;
		if (!insecureHttpWarned && this.baseUrl.startsWith("http://") && !isLoopbackUrl(this.baseUrl)) {
	...
	private async getJson<T>(path: string, options: { timeoutMs?: number } = {}): Promise<T> {
		...
		if (!response.ok) {
			throw new CortexApiError(`GET ${path} failed with HTTP ${response.status}`, response.status, text);
		}
	 ...
	 */
	async postJson<T>(path: string, payload: unknown, options: { agent?: string; timeoutMs?: number } = {}): Promise<T> {
		...
		const text = await response.text();
		if (!response.ok) {
			throw new CortexApiError(`POST ${path} failed with HTTP ${response.status}`, response.status, text);
		}
	...
	async deleteJson<T>(path: string, options: { timeoutMs?: number } = {}): Promise<T> {
		...
		const text = await response.text();
		if (!response.ok) {
			throw new CortexApiError(`DELETE ${path} failed with HTTP ${response.status}`, response.status, text);
		}
		if (!text.trim()) return undefined as T;
		return JSON.parse(text) as T;
	}
	...
	/**
	 * Semantic search over the project memory (cortex-api POST /search).
	 * Returns the raw result rows — the caller shapes them for the model.
	 */
	async search(query: string, limit = 5): Promise<Array<{ id: string; text: string; source: string; score: number }>> {
		const res = await this.postJson<{ results?: Array<{ id: string; text: string; source: string; score: number }> }>(
			"/search",
			{ query, limit },
		);
		return res.results ?? [];
	...
	/** Record a learning into the project memory (cortex-api POST /learnings). */
	async recordLearning(learning: string): Promise<void> {
		await this.postJson("/learnings", { learning });
	}
	 ...
	 */
	async *streamEvents(options: StreamEventsOptions = {}): AsyncGenerator<CortexStreamItem> {
		const {
			count = 50,
			pingSeconds = 15,
			signal,
		...
		while (!signal?.aborted) {
			try {
				...
				const response = await this.fetchImpl(`${this.baseUrl}/events?${params.toString()}`, {
					headers: this.headers({
				...
				if (!response.ok) {
					...
					throw new CortexApiError(`GET /events failed with HTTP ${response.status}`, response.status, body);
				}
				const contentType = response.headers.get("content-type") ?? "";
				if (!contentType.includes("text/event-stream")) {
					throw new Error(`GET /events returned unexpected content-type "${contentType}"`);
				}
				if (!response.body) {
				...
				if (failure instanceof CortexApiError && failure.status >= 400 && failure.status < 500) {
					throw failure;
				}
				attempt += 1;
