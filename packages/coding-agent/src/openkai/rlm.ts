...
import { type CompletionRequest, complete } from "./fusion/complete.js";
...
export class RlmRegistry {
	...
	spawnChild(streamFn: StreamFunction<Api>, model: Model<Api>, request: CompletionRequest): RlmSpawnHandle {
		this.generation += 1;
	...
	childUsageAttribution(): {
		totalTokens: number;
		children: Array<{ childId: string; model: string; totalTokens: number }>;
	} {
		let totalTokens = 0;
