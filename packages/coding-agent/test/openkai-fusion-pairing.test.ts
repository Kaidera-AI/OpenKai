...
import { candidateKey, postureBucket, suggestPair, type PairCandidate } from "../src/openkai/pairing";
...
describe("E022 Inc 03: scorer-driven pairing", () => {
  ...
  test("a configured pair is honoured verbatim", () => {
    const rec = suggestPair(TWO_PROVIDERS, TWO_PROVIDERS[0], { bucket: "medium" }, {
...
describe("E022 Inc 03: pair config round-trip", () => {
  test("write then read returns the pair", async () => {
    await writeOpenkaiConfig({ fusion: { pair: { architect: "nvidia/llama-70b", builder: "openrouter/nemotron-30b" } } });
