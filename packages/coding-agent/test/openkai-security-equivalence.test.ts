...
describe("F4/F10 class: deny floor containment", () => {
	...
	test("outside-cwd containment catches traversal outside temp", () => {
		// cwd OUTSIDE the system temp dir: containment is strict there.
		const cwd = realpathSync(path.join(import.meta.dir, "..", "src"));
		expect(outsideCwd(cwd, "../escape.txt")).toBe(true);
		expect(outsideCwd(cwd, "openkai/gate-floor.ts")).toBe(false);
		expect(outsideCwd(cwd, "/etc/passwd")).toBe(true);
	});

	test("system temp is exempt scratch — upstream tooling sandboxes there (CI contract)", () => {
		const cwd = realpathSync(path.join(import.meta.dir, "..", "src"));
		const scratch = path.join(tmpdir(), "pi-sdk-tool-activation-x", "runtime-write.txt");
		expect(outsideCwd(cwd, scratch)).toBe(false);
		// The DENY_FLOOR secret patterns still apply inside temp: exemption
		// narrows containment, never the secret globs.
		expect(floorMatchFor(path.dirname(scratch), path.join(path.dirname(scratch), ".env"))).toBeDefined();
		expect(floorMatchFor(cwd, path.join(tmpdir(), "staging", "id_ed25519"))).toBeDefined();
	});
});
