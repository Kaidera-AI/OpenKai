# Pi (Pi (upstream)) + Oh My Pi (omp) — findings for OpenKai

**Date:** 2026-08-14
**Agent:** PiOmpResearch
**Scope:** (a) four local KOS research docs on the pi lane; (b) `Pi (upstream)` package inventory and extension surface; (c) how omp extends pi; (d) build-on-top options for OpenKai.

---

## 1. What it is

**Pi (upstream)** (now resolved as `earendil-works/pi`; `github.com/Pi (upstream)` redirects) is Mario Zechner's TypeScript AI-agent toolkit monorepo: a unified multi-provider LLM API (`pi-ai`), an agent runtime (`pi-agent-core`), a differential-rendering terminal UI library (`pi-tui`), and an interactive coding-agent CLI (`pi-coding-agent`). Maturity: ~90.4k stars, 11.2k forks, very active; monorepo version 0.84.2 at time of writing; MIT licence.

**Oh My Pi (omp)** (`omp (upstream)`, upstream homepage) is a hard **source fork** of Pi (upstream) — not an extension package — re-published under the `@kaidera/openkai-*` npm scope with ~80k lines of Rust N-API natives, Bazel build, subagents, LSP/DAP, browser, memory, and a Claude Code-compatible plugin marketplace. Maturity: ~24.8k stars, active; locally installed as omp 17.3.4 (compiled Mach-O binary via Homebrew tap `upstream tap`); MIT licence. This session runs on omp.
...
**Pi (upstream)** (`packages/`, TypeScript, Node ≥ 22.19, ESM, Bun-compilable to standalone binaries via `scripts/build-binaries.sh`):
...
**omp** (`omp (upstream)`): same monorepo skeleton forked — `packages/{ai, agent, coding-agent, tui}` plus omp-specific `catalog`, `natives` (N-API over six Rust crates: pi-shell, pi-natives, pi-ast, pi-iso, pi-voice, pi-walker, plus vendored brush-shell + 67 in-process CLI utilities), `mnemopi` (SQLite memory engine), `snapcompact`, `hashline` (line-anchored edit format), `browser-relay`, `collab-web`, `metaharness`, `stats`, `omptype` (ArkType-compatible validator), `utils`, `wire`. Distributed as compiled standalone binaries and `@kaidera/openkai-pi-coding-agent` on npm. Four entry points: TUI, `omp -p`, `omp --mode rpc` (plus `--mode rpc-ui`), and `omp acp` (Agent Client Protocol for editors). Config/state under `~/.omp/agent/` (SQLite-backed: `agent.db`, `history.db`, `models.db` — observed locally).
...
- Local installs verified: `omp/17.3.4` at `/opt/homebrew/Cellar/omp/17.3.4/bin/omp` (Mach-O arm64, brew tap `upstream tap`, MIT); `pi 0.84.2` at `~/.npm-global/bin/pi`; `~/.omp/agent/` contains `agent.db`, `history.db`, `models.db`, `config.yml`.

**Pi (upstream) / earendil-works/pi (MIT):**
- Repo (redirect target, stars/files): https://github.com/Pi (upstream) → https://github.com/earendil-works/pi
- Packages listing: https://github.com/Pi (upstream)/tree/main/packages
- pi-ai README (providers, factories, handoffs): https://raw.githubusercontent.com/Pi (upstream)/main/packages/ai/README.md
- pi-ai exports: https://raw.githubusercontent.com/Pi (upstream)/main/packages/ai/package.json
- pi-agent-core README (loop, events, hooks): https://raw.githubusercontent.com/Pi (upstream)/main/packages/agent/README.md
- pi-tui README (renderers, overlays, components): https://raw.githubusercontent.com/Pi (upstream)/main/packages/tui/README.md and package.json
- pi-coding-agent README (modes, extensions, pi packages, CLI): https://raw.githubusercontent.com/Pi (upstream)/main/packages/coding-agent/README.md
- Extensions doc (ExtensionAPI, events, ctx.ui): https://raw.githubusercontent.com/Pi (upstream)/main/packages/coding-agent/docs/extensions.md
- SDK doc (`createAgentSession`, `AgentSessionRuntime`): https://raw.githubusercontent.com/Pi (upstream)/main/packages/coding-agent/docs/sdk.md
- Session format v3: https://raw.githubusercontent.com/Pi (upstream)/main/packages/coding-agent/docs/session-format.md
- pi-coding-agent exports: https://raw.githubusercontent.com/Pi (upstream)/main/packages/coding-agent/package.json
...
**omp / omp (upstream) (MIT):**
- Repo + README (fork statement, natives, roles, SDK exports, entry points): https://github.com/omp (upstream)
- Packages: https://github.com/omp (upstream)/tree/main/packages
- omp extensions doc: https://raw.githubusercontent.com/omp (upstream)/main/docs/extensions.md
- omp marketplace doc: https://raw.githubusercontent.com/omp (upstream)/main/docs/marketplace.md
- Docs index (extension-loading, custom-tools, porting-from-Pi (upstream), rpc, sdk, tui-*): https://github.com/omp (upstream)/tree/main/docs
- Homebrew formula (binary provenance, MIT): https://raw.githubusercontent.com/upstream/homebrew-tap/HEAD/Formula/omp.rb
