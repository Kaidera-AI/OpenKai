...
1. **Lineage convergence.** Pi (upstream) is the common ancestor of both omp (our runtime) and prime-agent. pi-ai (30+ providers, OAuth, unified usage/cost) and pi-tui (differential-rendering TUI library) are published MIT npm packages with supported embedder surfaces. OpenKai does not need to fork anything.
...
- 2026-08-21 — **TUI design practices + live gap audit (ren@openkai):** `2026-08-21-openkai-tui-design-practices-gap-audit.md` — reconciles the stale 2026-08-18 research handoff against the current tree. Diff, Mermaid, mouse, theme packs, status hierarchy, testing, and served-TUI rendering already exist; the remaining product work is an explicit size/capability matrix, an optional wide-screen context drawer, responsive split/stacked diffs, exact Kaidera tokens, `NO_COLOR`/16-colour/accessibility fallbacks, and delayed/coalesced busy rendering. Product recommendation: chat-first, context-on-demand; terminal-native tonal depth rather than literal web neomorphism.
