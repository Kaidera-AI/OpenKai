...
- Prime Agent is not a from-scratch harness: it *is* pi (package names, `piConfig`, README acknowledgement). Anything KOS knows about Pi (upstream) internals transfers; the fork's value-add is daemon + kernel + harness state.
